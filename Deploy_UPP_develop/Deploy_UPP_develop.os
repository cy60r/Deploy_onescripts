...
Процедура ИнициализироватьЛог()
	...
	Логирование.ЗаписатьСтрокуЛога("Начало выполнения деплоя");
	...
	Если Не Отказ Тогда
		...
		Для каждого Строка Из ТаблицаВерсий Цикл
			...
			Если Не Отказ Тогда
				...
				ВыполнитьДымовыеТесты(Отказ, Строка.Автор, ТекстФайлаАвторов);
				ОтправитьГИТ(Отказ);
...
Процедура ВыполнитьДымовыеТесты(Отказ, Автор, ТекстФайлаАвторов)
