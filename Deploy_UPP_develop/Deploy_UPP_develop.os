...
Процедура ВыполнитьСканирование(Отказ, КлючПроекта, Пользователь)
	...
	Если Не Отказ Тогда
		...
		ПараметрКодировка = 	СтрШаблон("-Dsonar.sourceEncoding=%1", 	Параметры["Кодировка"]);
		ПараметрИсточник = 		СтрШаблон("-DSonar.sources=%1", 		".");
		ПараметрАдресХоста = 	СтрШаблон("-Dsonar.host.url=%1", 		Параметры["АдресХоста"]);
		ПараметрФильтр = 		СтрШаблон("-Dsonar.inclusions=%1", 		Параметры["ФильтрСкана"]); 
