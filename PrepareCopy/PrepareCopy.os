...
Процедура ПрочитатьПараметрыИзФайла()
	...
	Параметры = ЧтениеПараметров.Прочитать(ОбъединитьПути(ТекущийКаталог(), "PrepareCopy.json"));
	...
	Если Не Отказ Тогда
		...
		Для каждого Строка Из ТаблицаВерсий Цикл
			...
			Если Не Отказ Тогда
				...
				СделатьКоммитВерсии(Отказ, Строка);
...
Функция НомерТекущейВерсии()
	...
	РезультатЧтения = ПроцессорXML.ПрочитатьИзФайла(ОбъединитьПути(Параметры["ОсновнойКаталог"], "VERSION"));
...
Процедура СделатьКоммитВерсии(Отказ, ДанныеВерсии)
	
	Если Не Отказ Тогда

		АвторКоммита = ЗначениеИзТекста(ДанныеВерсии.Автор, ПрочитатьФайл(ОбъединитьПути(Параметры["ОсновнойКаталог"], "AUTHORS")));
...
Процедура ВыполнитьСканирование(Отказ, КлючПроекта, Пользователь)
	...
	Если Не Отказ Тогда
		...
		ПараметрыСканера = СтрШаблон("%1 %2 %3 %4 %5 %6", 
		СтрШаблон("-Dsonar.projectKey=%1", 		КлючПроекта),
		СтрШаблон("-Dsonar.login=%1", 			Токен),
		СтрШаблон("-Dsonar.sourceEncoding=%1", 	Параметры["Кодировка"]),
		"-DSonar.sources=.",
		СтрШаблон("-Dsonar.host.url=%1", 		Параметры["АдресХоста"]),
		СтрШаблон("-Dsonar.inclusions=%1", 		Параметры["ФильтрСкана"]));
		...
		Иначе
			...
			Отказ = Истина;
			ОбработатьИсключение("Ошибка при выполнении sonar сканирования!");
