...
Процедура ВыполнитьДымовыеТесты(Отказ) Экспорт
	...
	Если Не Отказ Тогда
		...
		СписокТестов = Параметры["СписокТестов"];
		СписокОтчетов = Новый СписокЗначений();

		Мониторинг.Информация("Начало выполнения тестов");
		Мониторинг.УвеличитьУровень();
		Для каждого Тест Из СписокТестов Цикл
			...
			ПараметрКонфига = СтрШаблон("xddConfig %1;", ДопПараметрОбработки(Тест.ИмяКонфига));
			...
			Если КодВозврата = 0 Тогда
				...
				Мониторинг.Отладка(СтрШаблон("Дымовые тесты %1 выполнены", Тест.Наименование));
				ЗаголовокФайлаОтчета = ЗаголовокОтчетаРезультатаТестов(ИмяФайлаОтчета);
				
				Если ЗначениеЗаполнено(ЗаголовокФайлаОтчета) Тогда
					
					ТекстСообщения = ТекстСообщения + ЗаголовокФайлаОтчета;
					СписокОтчетов.Вставить(ИмяФайлаОтчета);

				КонецЕсли;
	
			Иначе
				
				Мониторинг.Ошибка(СтрШаблон("Ошибка выполнения дымовых тестов %1", Тест.Наименование));
		...
		Если ЗначениеЗаполнено(ТекстСообщения) Тогда
			...
			ОтправитьОтчетТестирования(ТекстСообщения, СписокОтчетов);
...
Функция ЗаголовокОтчетаРезультатаТестов(ИмяФайла)
...
Процедура ОтправитьОтчетТестирования(ТекстСообщения, СписокОтчетов)
	...
	СтруктураСообщения.Вложения = СписокОтчетов;
